...
<a @isset($menu['children']) v-on:click="tmenu_{{$menu['key']}} = !tmenu_{{$menu['key']}}" @else href="{{ url('/'.$menu['key']) }}" @endisset>
    ...
    @isset($menu['children'])
    <span v-html="tmenu_{{$menu['key']}}"></span>
    <div class="thidden pl-{{$margin}}" v-bind:class="{block:tmenu_{{$menu['key']}}}">
